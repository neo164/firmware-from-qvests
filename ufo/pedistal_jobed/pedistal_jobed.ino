...
boolean flag = false, svet = false, fl_strip = false, rounder_flag = false, pressure_st = false, key = false, pink = false, green = false;
...
int delayval = 15, delayval_g = 25; // delay for half a second
...
void loop() {
  ...
  if (key){  
    //if((outputValue0 >=120 &&  outputValue1 >=130 && outputValue2 >=128 && outputValue3 <= 100 ) || (!st_sw11)) {
      //if( (outputValue0 >=68 && outputValue0 <=77) &&  (outputValue1 >=130 && outputValue1 <=140) && (outputValue2 >=121 && outputValue2 <=127) && (outputValue3 >=92 && outputValue3 <= 98) || (!st_sw11) ) {
      if( (outputValue0 >=72 && outputValue0 <=80) &&  (outputValue1 >=118 && outputValue1 <=127) && (outputValue2 >=90 && outputValue2 <=97) && (outputValue3 >=90 && outputValue3 <= 97) || (!st_sw11) ) {

       //новые фигуры - крепкие 
  ...
  if ((pressure_st) || (!st_sw9)) {
    ...
    //if(rounder_flag) rounder_flag = false;
  ...
  else if ((digitalRead(led23) && digitalRead(led25) && digitalRead(led27) && digitalRead(led29) && digitalRead(led31) && digitalRead(led33) && digitalRead(led35) && digitalRead(led37) && rounder_flag) || (!st_sw10))//roll_changer
  {
    if (!key) {
      //открытие двери
...
void ledstrip_g() {//green
  ...
  //else {
  {
    for (int i = NUMPIXELS; i >= 0; i--) {
      ...
      delay(delayval_g); // Delay for a period of time (in milliseconds).
