...
void setup()
{
  ...
  Timer1.initialize(300000); // set a timer of length 100000 microseconds 
...
void timer() {
  ...
  if (din8_st) {  //замок первой двери
    ...
    //-----SET8_TRACK--------
    if (!track_8 && fl_unl_poly) { //финальная музыка
      delay(10);
      mp3tf.setVolume (20);
      delay(50);
      mp3tf.play (8);
...
//-----SET_10_TRACK--------NEW AFTER NY!!!!--------
if (!track_10 && fl_fakel) { 
      delay(10);
      mp3tf.setVolume (23);
      delay(50);
      mp3tf.play (10);
