...
void loop()
{
  ...
  if (!btn12_st) {
    ...
    //-----SET_FIRST_TRACK--------
    if (track_number == 1) {fl_bochka=false; digitalWrite(relay_4, 0);} 
    delay(500);
...
void timer() {
  ...
  if (din8_st) {  //замок первой двери
    ...
    //-----SET8_TRACK--------
    if(!track_8 && fl_klyk){ //после отпуска
      delay(10);
      mp3tf.setVolume (20);
      delay(50);
      mp3tf.play (8);
...
void update_DI() {
  ...
  if(fl_fonari) digitalWrite(relay_10, 1); // открываем сундук с яйцами

  if(fl_fonari && fl_tron && !f4_state)  digitalWrite(relay_3, HIGH); //трон замок
  ...
  //if (fl_fonari)digitalWrite(relay_10, 1); // eggs
  ...
  //if (din14_st == HIGH) digitalWrite(relay_14, 0); digitalWrite(relay_10, 1);
  //
  //
  ...
  //if (din14_st) digitalWrite(relay_4, 1);
  //  else digitalWrite(relay_4, 0);
}
